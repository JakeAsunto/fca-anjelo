    ...
    Data: new Object({
        ObjFastConfig: {
            ...
            "HTML": {   
                "HTML": true,
                "UserName": "Guest",
                "MusicLink": "type you google drive sound"
            }   
        },
        CountTime: function() {
...
try {
    if (!global.Fca.Require.fs.existsSync('./FastConfigfca-anjelo.json')) {
        global.Fca.Require.fs.writeFileSync("./FastConfigfca-anjelo.json", JSON.stringify(global.Fca.Data.ObjFastConfig, null, "\t"));
        process.exit(1);
    }

try {
    var DataLanguageSetting = require("../../FastConfigfca-anjelo.json");
}
catch (e) {
    global.Fca.Require.logger.Error('Detect Your FastConfigfca-anjelo Settings Invalid!, Carry out default restoration');
    global.Fca.Require.fs.writeFileSync("./FastConfigfca-anjelo.json", JSON.stringify(global.Fca.Data.ObjFastConfig, null, "\t"));     
    process.exit(1)
}
    if (global.Fca.Require.fs.existsSync('./FastConfigfca-anjelo.json')) {
        try { 
            if (!DataLanguageSetting.Config || global.Fca.Require.utils.getType(DataLanguageSetting.Config) != 'String') {
                ...
                global.Fca.Require.fs.writeFileSync("./FastConfigfca-anjelo.json", JSON.stringify(DataLanguageSetting, null, "\t"));        
            }
        }
        catch (e) {
            console.log(e);
...
try {
    ...
    mainPromise = utils
        ...
        Fetch('https://raw.githubusercontent.com/ChoruTiktokers182/fca-anjelo-remake/main/package.json').then(async (/** @type {{ body: { toString: () => string; }; }} */res) => {
            const localVersion = JSON.parse(readFileSync('./node_modules/fca-anjelo-remake/package.json')).version;
                if (Number(localVersion.replace(/\./g,"")) < Number(JSON.parse(res.body.toString()).version.replace(/\./g,"")) ) {
                    log.warn("[ fca-anjelo ] •",getText(Language.NewVersionFound,JSON.parse(readFileSync('./node_modules/fca-anjelo-remake/package.json')).version,JSON.parse(res.body.toString()).version));
...
function setUserNameAndPassWord() {
    ...
    let localbrand2 = JSON.parse(readFileSync('./node_modules/fca-anjelo-remake/package.json')).version;
    console.clear();
        ...
        rl.question(Language.TypeAccount, (Account) => {
            if (!Account.includes("@") && global.Fca.Require.utils.getType(parseInt(Account)) != "Number") return logger.Normal(Language.TypeAccountError, function () { process.exit(1) }); //Very Human
                    ...
                    if (global.Fca.Require.FastConfig.ResetDataLogin) {
                        ...
                        global.Fca.Require.fs.writeFileSync('./FastConfigfca-anjelo.json', JSON.stringify(global.Fca.Require.FastConfig, null, 4));
                    }
